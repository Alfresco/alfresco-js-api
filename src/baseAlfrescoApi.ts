...
export abstract class BaseAlfrescoApi extends LegacyAlfrescoApi implements Emitter, AlfrescoApiType {
    ...
    abstract initProcessClient(config: AlfrescoApiConfig): ProcessClient;
    abstract initOauth2Auth(config: AlfrescoApiConfig): Oauth2Auth;
    abstract initProcessAuth(config: AlfrescoApiConfig): ProcessAuth;
    abstract initContentAuth(config: AlfrescoApiConfig): ContentAuth;
    abstract initContentClient(config: AlfrescoApiConfig, servicePath: string): ContentClient;
    ...
    setConfig(config: AlfrescoApiConfig) {
        ...
        this.processClient = this.initProcessClient(this.config);
    ...
    private initAuth(config: AlfrescoApiConfig): void {
        if (this.isOauthConfiguration()) {
            if (!this.oauth2Auth) {
                this.oauth2Auth = this.initOauth2Auth(config);
            } else {
                this.oauth2Auth.setConfig(config, this);
            }
            ...
            if (!this.processAuth) {
                this.processAuth = this.initProcessAuth(config);
            } else {
                this.processAuth.setConfig(config);
            }
    ...
    private clientsFactory() {
        if (!this.contentPrivateClient) {
            this.contentPrivateClient = this.initContentClient(this.config, `/api/${this.config.tenant}/private/alfresco/versions/1`);
        } else {
            this.contentPrivateClient.setConfig(this.config, `/api/${this.config.tenant}/private/alfresco/versions/1`);
        }

        if (!this.contentClient) {
            this.contentClient = this.initContentClient(this.config, `/api/${this.config.tenant}/public/alfresco/versions/1`);
        } else {
            this.contentClient.setConfig(this.config, `/api/${this.config.tenant}/public/alfresco/versions/1`);
        }

        if (!this.authClient) {
            this.authClient = this.initContentClient(this.config, `/api/${this.config.tenant}/public/authentication/versions/1`);
        } else {
            this.authClient.setConfig(this.config, `/api/${this.config.tenant}/public/authentication/versions/1`);
        }

        if (!this.searchClient) {
            this.searchClient = this.initContentClient(this.config, `/api/${this.config.tenant}/public/search/versions/1`);
        } else {
            this.searchClient.setConfig(this.config, `/api/${this.config.tenant}/public/search/versions/1`);
        }

        if (!this.discoveryClient) {
            this.discoveryClient = this.initContentClient(this.config, `/api`);
        } else {
            this.discoveryClient.setConfig(this.config, `/api`);
        }

        if (!this.gsClient) {
            this.gsClient = this.initContentClient(this.config, `/api/${this.config.tenant}/public/gs/versions/1`);
        } else {
